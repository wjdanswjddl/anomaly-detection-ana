#!/usr/bin/env python3
"""
Load ICARUS-style npz files (key 'reco'), apply synthetic detector defects,
and save next to the source tree in sibling directories named
<original_dir>_<defect> (full reco) and <original_dir>_<defect>_defect
(defect-only map: modified minus original, same shape as reco).

Defects mirror MakeDetectorFeatures.ipynb: bad_wire, coh_noise, charge_tail.
Locations and defect hyperparameters are randomized per frame for variety.
"""
...
# ICARUS zero-pole cancellation decay (same notebook constant)
...
def apply_bad_wire(frame: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    if mode == "uniform":
        out[:, col] = rng.uniform(-3 * maxval, 3 * maxval, size=h).astype(np.float32)
    else:
        scale = rng.uniform(0.2, 0.8) * maxval
...
def apply_coh_noise(frame: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    noise = wave_packet(
        ...
        amplitude=float(rng.uniform(0.08, 0.22)) * sig,
...
def apply_charge_tail(frame: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    n_pulses = int(rng.integers(5, 15))
